...
for i=1:size(x,2)
    ...
    switch type
        case {'bilateral', 'man'}
            ...
            hold on
            patch([pos(i)-f fliplr(pos(i)+f)], [uc fliplr(uc)], 0.97*col(i,:),'edgecolor','k','facealpha',0.3)
            ...
            patch([pos(i)-boxw(i)/8 pos(i)+boxw(i)/8 pos(i)+boxw(i)/8 pos(i)-boxw(i)/8 pos(i)-boxw(i)/8], [mu(i)-sigma(i) mu(i)-sigma(i) mu(i)+sigma(i) mu(i)+sigma(i) mu(i)-sigma(i)], col(i,:),'edgecolor','k','facealpha',0.35,'HandleVisibility','off')
        ...
        case 'left'
            ...
            hold on
            patch(pos(i)-f, uc, 0.97*col(i,:),'edgecolor','none', 'LineWidth', 1.5,'facealpha',0.3,'HandleVisibility','off')
            patch([pos(i) pos(i)-boxw(i)/2 pos(i)-boxw(i)/2 pos(i)-boxw(i)/4 pos(i)-boxw(i)/2 pos(i)-boxw(i)/2 pos(i) pos(i)], [q(2,i) q(2,i) notch(1,i) q(3,i) notch(2,i) q(4,i) q(4,i) q(2,i)], 0.97*col(i,:),'edgecolor','k','facealpha',0.8,'HandleVisibility','off')
            patch([pos(i)-boxw(i)/8 pos(i) pos(i) pos(i)-boxw(i)/8 pos(i)-boxw(i)/8], [mu(i)-sigma(i) mu(i)-sigma(i) mu(i)+sigma(i) mu(i)+sigma(i) mu(i)-sigma(i)], col(i,:),'edgecolor','none','facealpha',0.5,'HandleVisibility','off')
            ...
            plot(pos(i)-boxw(i)/20, mu(i),'+','color','k','linewidth',1,'HandleVisibility','off')
